	...
	$("#tt").datagrid(
			{
				...
				width : $("#body").width(),
				idField : 'id',
				//url :'<c:if test="${user.role==1}">EquiPayListAction_listEquiPaylist</c:if>',
				//url : '${user.role==1}?EquiPayListAction_listEquiPaylist:EquiPayListAction_listAllPaylist',			
				url : 'EquiPayListAction_listEquiPaylist',
		    ...
		    $("#addpaylist").on("click",function(){
		   		$("#add_form").form("submit",{    //表单提交
		   			url:"EquiPayListAction_addEquiPaylist",
			...
			$("#paylist_saveAndEdit").on("click",function(){
				$("#update_form").form("submit",{
					url:"EquiPayListAction_updatePaylist",
					success:function(result){
						if(result){
				...
				if (data.length > 0 && data != null) { // 判断是否选中
					$.messager.confirm("提示","确实要删除吗？",function(r) {
						if (r) { // 如同 r==true
							...
							$.post("EquiPayListAction_deletePaylist",{id : id}, // 将 id 传到 action,在action中根据id获取对象
			...
			$("#query").on("click",function(){
				...
				$('#tt').datagrid('options').url = "EquiPayListAction_listEquiPaylist?queryData="+encodeURI(encodeURI(queryData));
				$('#tt').datagrid('reload');
			});
	...
	<div id="body" region="center">
		...
		<div id="tt_btn">
			<a href="javascript:void(0)" id="save" class="easyui-linkbutton"
				iconCls="icon-add" plain="true">预采购申请
			</a> 
		...
		<!-- 添加的弹出框 -->
		<div id="add_win" class="easyui-window" title="预采购申请"
				...
				<table width="280" border="0" align="center" cellpadding="3">
					...
					<tr>
						<td align="right"><label for="applicant"><span
								class="x">*</span>申请人</label></td>
						<td><input type="text" name="applicant" id="applicant"
					...
					<tr style="display: none">
						<td align="right"><label for="ifpass"><span
								class="x">*</span>是否通过</label></td>
						<td><input type="text" name="ifpass" id="ifpass" 
								  value="否" readonly="readonly"/></td>
					</tr>
					<tr>
						<td align="right"><label for="applydate"><span
								class="x">*</span>申请时间</label></td>
						<td><input type="date" name="applydate" id="applydate" readonly="readonly"
						 value="<%=new SimpleDateFormat("yyyy-MM-dd").format(new Date())%>" /></td>
					</tr>
					
					<tr style="display: none">  <!-- 不显示默认值 -->
						<td><input type="date" name="passdate" id="passdate" value="" /></td>
					</tr>
			...
			<form id="update_form" action="" method="post">
				<table width="280" border="0" align="center" cellpadding="3">
					...
					<tr>
						<td align="right"><label for="ifpass"><span
								class="x">*</span>是否通过</label></td>
						<td><input type="text" name="ifpass" /></td>
					</tr>
					<tr>
						<td align="right"><label for="applydate"><span
								class="x">*</span>申请时间</label></td>
						<td><input type="text" name="applydate" class="easyui-datebox" required="required"/></td>
					</tr>
					
					<c:if test="${user.role==1 }">
						<tr>
							<td align="right"><label for="passdate"><span class="x">*</span>申请时间</label></td>
							<td><input type="text" name="passdate" class="easyui-datebox" required="required"/></td>
						</tr>
					</c:if>
